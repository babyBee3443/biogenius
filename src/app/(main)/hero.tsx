...
const HeroComponent: React.FC<HeroProps> = ({ articles }) => {
  ...
  React.useEffect(() => {
    // This effect handles the case where the current index might be out of bounds
    // if the displayArticles array changes (e.g., due to filtering or data updates).
    if (displayArticles.length > 0 && currentIndex >= displayArticles.length) {
      setCurrentIndex(0); // Reset to the first slide if current is out of bounds
    } else if (displayArticles.length === 0 && currentIndex !== 0) {
      setCurrentIndex(0); // Reset if no articles to display
    }
           ...
           <section className="relative overflow-hidden bg-gradient-to-br from-indigo-100 to-blue-100 dark:from-indigo-900/30 dark:to-blue-900/30 h-[50vh] md:h-[60vh] flex items-center justify-center text-center mb-16 rounded-lg shadow-inner border border-primary/10">
              <div className="container relative z-10 px-4">
                 ...
                 >
                    ...
                    <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-3">BiyoHox'a Özel İndirimler!</h2>
  ...
  // Fallback if currentArticle is somehow undefined after checks (should not happen)
  if (!currentArticle) {
     // Optionally, render a loading state or a default ad here as well
     return (
        <div className="relative overflow-hidden bg-gray-100 dark:bg-gray-800 h-[50vh] md:h-[60vh] flex items-center justify-center text-center mb-16 rounded-lg shadow-inner">
            <p className="text-muted-foreground">Hero içeriği yükleniyor veya mevcut değil...</p>
        </div>
     );
  }
  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-background to-secondary/30 h-[50vh] md:h-[60vh] flex items-center justify-center text-center mb-16 rounded-lg shadow-inner">
      ...
      <div className="container relative z-10 text-white dark:text-gray-100 px-4">
        <div className="relative h-48 md:h-40"> {/* Adjusted height for content area within hero */}
          <AnimatePresence initial={false} mode="wait">
            <motion.div
...
export const Hero = React.memo(HeroComponent); // Ensure component is memoized
export default Hero;
